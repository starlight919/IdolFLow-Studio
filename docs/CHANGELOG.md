...
## [2026-08-21] - Anchor 任务/运行按文件夹分组 + 排序 + 卡片布局对齐

### ✨ 新增
- **Anchor 任务/运行按任务文件夹（data dir）分组**：「Anchor 任务」与「Anchor 运行进度」均复用视频的 `task-group` 折叠分组层级（📁 文件夹 · 数量 · 可折叠/展开），与视频「已保存任务」「运行」展示完全一致；运行记录按 `task_id`（即 data_dir）归组，新增通用的 `renderGroupedRuns` 分组渲染工具（`utils.js`）
- **Anchor 列表排序**：任务与运行列表头部均加入四态排序按钮（时间 ↓ 最新在前 / 时间 ↑ 最早在前 / 名字 A-Z / 名字 Z-A），按钮实时高亮当前模式；组间排序时间模式按组内最新活动、名字模式按文件夹名

### 🎨 交互优化
- **Anchor 任务卡片横排对齐运行卡片**：卡片改为与视频运行卡片同款横排布局（`.task-main` + `.task-actions` + 右上角 ✕ 删除），并抽 `anchorTaskCard` 统一渲染
- **分组折叠状态持久化**：Anchor 任务分组的折叠/展开状态记 `localStorage`，跨会话保留（与视频任务分组一致）
