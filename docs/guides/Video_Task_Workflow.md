...
> 素材选择：Anchor 与参考音视频以「标签 chips」展示，点「✕」取消选中（不删除磁盘文件）。上传素材后文件持久化到任务文件夹，之后可直接通过「选择文件」从 `references/`（参考音视频）或 `anchors/selected/`（Anchor 图片）目录引用，无需重复上传。
...
切换到「审核」tab 查看已完成候选视频，用视频播放器控件自带的下载功能保存。
