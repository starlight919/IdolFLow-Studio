...
所有素材统一存放在任务数据目录（`data/<数据目录>/`）下，后端按「相对任务目录的路径」灵活解析：

| 素材 | 约定位置 | 来源 |
|------|---------|------|
| Anchor 图片 | `anchors/` | 上传（固定落点）/ 用户手动放置 |
| Anchor 候选图 | `anchors/selected/` | 现场生成（内部目录，用户无需直接操作） |
| 参考音视频 | `references/` | 上传（固定落点）/ 用户手动放置 |
| 任意素材 | 任务目录根目录 | 用户手动放置（后端同样可解析） |

> **上传固定落点**：通过浏览器「上传素材」时，图片固定存到 `anchors/`，音视频固定存到 `references/`。
>
> **手动放置**：用户自行往任务目录放素材时，放到根目录、`anchors/` 或 `references/` 子目录均可被识别（后端按相对路径灵活解析）。
>
> **引用路径**：所有 `file` 字段统一存「相对任务目录的路径」，可带子目录前缀（如 `anchors/xxx.png`）或直接是根目录文件名（如 `xxx.mov`），后端 `task_dir + file` 解析。
...
- 「选择文件」默认进入约定子目录（Anchor → `anchors/`，音视频 → `references/`）；若子目录不存在，自动逐级回退到任务根目录，仍能看到根目录下的素材。
...
任务保存时，`file` / `audio_file` 字段统一存**「相对任务数据目录（`data/<数据目录>/`）的路径」**，可带子目录前缀（如 `anchors/xxx.png`、`references/xxx.mp3`）或直接是根目录文件名。后端按 `task_dir + file` 解析绝对路径，找不到时再回退到 `data_root + file` 查找。
