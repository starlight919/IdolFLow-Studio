...
```
data/<数据目录>/
├── anchors/                  # Anchor 图片（上传 / 手动放置，图片唯一落点）
...
| 素材 | 约定位置 | 来源 |
|------|---------|------|
| Anchor 图片 | `anchors/` | 上传（固定落点）/ 用户手动放置 |
| Anchor 候选图 | `anchors/selected/` | 现场生成（内部目录，用户无需直接操作） |
...
- 级联清理范围（不可恢复）：
  - 任务定义 `tasks/*.json`
  - Anchor 图片 / 候选 / 精选图（`anchors/` 整个子目录）
  - 参考音视频（`references/`）
...
切换到「审核」tab 查看已完成候选视频：每个候选可在线播放，点视频右下角的「⋯」（更多选项）即可下载到本地。
