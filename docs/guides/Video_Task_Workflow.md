...
- 已填写但目录缺素材时，提示分两类分别给出引导：
  - 缺 Anchor 图片 → 提示可「现场生成」、下方上传、或手动放入 `anchors/`；
  - 缺参考音视频 → 提示可下方上传、或手动放入 `references/`。
- 目录内素材齐全时，提示自动隐藏。
...
- 切换任务文件夹时，若已有选中的 Anchor / 参考音视频，会先弹窗提醒（切换会清空已选素材），确认后才切换并清空；取消则保持原状。
- 这避免了「上一个文件夹的素材残留到新文件夹」的数据串扰。
