...
data/<任务文件夹>/                  # 素材、任务配置、Anchor 模块
  ...
  ├── seedance/                   # 云端资产缓存（assets.json、append-only 台账 asset_ledger.jsonl）
  └── tasks/                      # Video 任务配置（可有多个）
runtime/outputs/<任务文件夹>/        # 生成的视频
runtime/work/<任务文件夹>/<任务>/     # 临时文件（按任务隔离）
```
...
| 模式 | 传视频 | 传音频 | 歌词 | 用途 |
|------|--------|--------|------|------|
| `lip_sync` 对口型 | 可选 | 可选 | **必填** | 严格对齐逐字嘴形，**可纯音频驱动** |
| `dance_lip_sync` 口型+动作 | 强制 | 可选 | **必填** | 同步模仿口型与动作编排 |
| `motion` 模仿动作 | 强制 | 不传 | 不需要 | 舞蹈/手势舞，不约束口型 |
| `custom` 自由定制 | 否 | 可选（可选） | 不需要 | 高度自由的定制生成：Prompt + 可选参考图/视频/音频，直调 Seedance 2.5 |

> 💡 对口型 / 口型+动作模式的歌词为**必填**：未填时点「启动生成」会立即提示、无法进入生成（保存草稿不强制，可先补 Anchor 后再填歌词）。
>
> 💡 `custom`（自由定制）是自由度最高的生成方向：**Prompt（必填）与参考图 / 视频 / 音频都在「素材」面板的「创意 Prompt 与 参考」区块一起填写**，即可直接调用 Seedance 2.5 文生 / 图生 / 视频生视频；可选上传参考图（也可点「现场生成」用 GPT Image 2 现场生成候选图、从 Anchor 生成器「用作参考图」回填）、参考视频、参考音频（都不传也行）。参考视频 / 音频仅作为「参考形式」传入，**不做口型 / 动作对齐、不做 seg 切片与回灌**——与其他三种模式的主要区别就在这里。高级视频设置（分辨率、宽高比、生成音频、水印、输出格式）依旧可用；内置人物 / 风景 / 氛围 / 动物 / 美食 / 产品等多类 Prompt 模板，勾选即填。详见 [docs/guides/Video_Task_Workflow.md](docs/guides/Video_Task_Workflow.md#自由定制custom模式)。
